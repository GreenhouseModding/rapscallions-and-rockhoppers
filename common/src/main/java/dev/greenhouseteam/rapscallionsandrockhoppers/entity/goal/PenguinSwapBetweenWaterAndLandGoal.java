...
public class PenguinSwapBetweenWaterAndLandGoal extends RandomStrollGoal {
    private static final int SWAP_TO_WATER_CHANCE = 100;
    private static final int SWAP_TO_LAND_CHANCE = 200;
    ...
    public boolean canUse() {
        if ((!this.mob.isInWaterOrBubble() && this.mob.getRandom().nextInt(SWAP_TO_WATER_CHANCE) == 0
            || this.mob.isInWaterOrBubble() && this.mob.getRandom().nextInt(SWAP_TO_LAND_CHANCE) == 0)) {
    ...
    protected Vec3 getPosition() {
        ...
        Optional<BlockPos> generatedPos = BlockPos.betweenClosedStream(new BoundingBox(-8, -6, -8, 8, 6, 8).moved(pointOfInterest.getX(), pointOfInterest.getY(), pointOfInterest.getZ())).filter(pos -> (((Penguin) this.mob).getPointOfInterest() == null || !GoalUtils.mobRestricted(this.mob, 8)) && !GoalUtils.isSolid(this.mob, pos) && (this.mob.isInWaterOrBubble() ^ GoalUtils.isWater(this.mob, pos))).map(BlockPos::immutable).min(Comparator.comparing(pos -> pos.distManhattan(this.mob.blockPosition())));
